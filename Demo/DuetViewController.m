...
@interface DuetViewController () <UITableViewDelegate, UITableViewDataSource>
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"Duet";
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    UILabel *headerLabel = [[UILabel alloc] initWithFrame:CGRectMake(16, 35, tableView.bounds.size.width, 18)];
        headerLabel.text = @"Who can Duet with your videos";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIImageView *display;
    NSString *stateName = [self.option_State objectForKey:@("Duet")];
    switch (indexPath.row) {
        case 0:
        {
            ...
            [c addSubview:label];
            if ([stateName intValue] == DUET_EVERYONE) {
        ...
        case 1:
        {
            ...
            [c addSubview:label2];
            if ([stateName intValue] == DUET_FRIENDS) {
                    display = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"choose"]];
            } else {
                display = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"not_choose"]];
            }
        ...
        case 2:
        {
            ...
            label.textColor = [UIColor colorWithRed:22/255.0 green:24/255.0 blue:35/255.0 alpha:1];
            [c addSubview:label];
            if ([stateName intValue] == DUET_ONLY_ME) {
                    display = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"choose"]];
            } else {
                display = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"not_choose"]];
            }
...
- (void)clickImage:(UITapGestureRecognizer*)sender
{
    ...
    switch (index) {
        case 0:
        {
            [self.option_State setValue:@(DUET_EVERYONE) forKey:@"Duet"];
            break;
        }
        case 1:
        {
            [self.option_State setValue:@(DUET_FRIENDS) forKey:@"Duet"];
            break;
        }
        case 2:
        {
            [self.option_State setValue:@(DUET_ONLY_ME) forKey:@"Duet"];
            break;
        }
        default:
            break;
    }
    [self.delegate duetChangeState:self];
    NSString *cachePatch = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0];
    NSString *filePath = [cachePatch stringByAppendingPathComponent:@"State.plist"];
